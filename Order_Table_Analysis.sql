...
select TO_CHAR(created_at,'HH24'/*('Month'/'Day'/'YYYY'/'Q')*/) as "Year",COUNT(*) "Amount_Of_Orders" 
from Orders
group by 1
order by 2 desc;
...
with TD as(
	select od.created_at Order_Time,ws.created_at Website_session,od.Price Price
	from Orders od 
	inner join Website_Session ws 
	USING(Website_Session_ID)),
   AVG_price as(
	select AVG(Price) AVG from TD
)
...
select User_id,COUNT(*) Amount_Of_Order
from orders od 
group by 1
order by 2 desc
)
...
--No correlation between Website_Session and Order_Time

--Every Order Has only One product 'Boooooooooring'
select user_id,ARRAY_AGG(p.product_name) from Orders s inner join Product p USING(Product_id)
group by 1
order by 1 desc;
...
select TO_CHAR(ws.Created_at,'HH24')Time_Of_Day,COUNT(O.Order_ID) Amount_Of_Orders,LPAD('*',TRUNC(COUNT(O.Order_ID)/100)::INT,'*') "Chart"
from Orders O 
inner join Website_Session Ws
using(Website_Session_ID)
group by 1
...
select distinct(P.Product_id),P.Product_name,AVG(O.Price) over (partition by p.product_name ) Avg_Price from Product P
inner join Orders O 
using (Product_id)
order by 3 desc;
--Most Expensive and Cheap Order
select distinct(O.Order_id),P.Product_name,AVG(O.Price) over (partition by p.product_name) Avg_Price from Product P
inner join Orders O 
using (Product_id)
order by 3 desc;
